  ...
  methods: {
    ...
    async install () {
      ...
      try {
        ...
        this.saEvent(
    `install_build__${this.$root.$data.product}_${this.$root.$data.release.version}_${this.$root.$data.release.variant}_${this.$root.$data.release.partition}`
        )
        const partition = this.$root.$data.release.partition
        const blob = new Inflate(this.$root.$data.zipBlob, { to: 'raw' })
        ...
        await this.device.flashBlob(
          ...
          (action, item, progress) => {
            const userAction = fastboot.USER_ACTION_MAP[action]
            const userItem =
              item === 'avb_custom_key'
                ? 'verified boot key'
                : item
